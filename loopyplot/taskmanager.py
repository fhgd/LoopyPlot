...
class Argument:
    ...
    @config
    def depends_on(self, param, sweeps=[], squeeze=[], concat=False):
        """Define parameter dependency for argument.
        ...
        >>> quad.args.offs.iterate(0, 10, 20)
        >>> quad.run(1)
        >>> quad.returns.as_table()
           x  offs   y
        0  1     0   1
        1  2     0   4
        2  3     0   9
        ...
        ...     return
        >>> task = Task(func)
        >>> task.args.a.iterate(0, 2, 4, 6)
        >>> task.args.b.depends_on(quad.returns.y, squeeze=['x'])
        >>> task.run(0)
        >>> task.run(0)
        >>> quad.run()
        >>> task.run()
        ...
        >>> task = Task(func)
        >>> task.args.a.depends_on(quad.args.offs)
        >>> task.args.b.depends_on(quad.returns.y, sweeps=quad.args.x)
        >>> task.run()
        >>> task.args.as_table(hide_const=False)
            a   b
        0  20  21
        1  20  24
        2  20  29
        3  20  36
        """
        squeeze = [arg.name for arg in param._task.args._get(squeeze)]
        sweeps = [arg.name for arg in param._task.args._get(sweeps)]
        if not sweeps:
            ptr = ParamPointer(param, squeeze)
            self._task.args._remove_sweeped_arg(self)
        else:
            ptr = ParamSweepPointer(param, sweeps, squeeze)
            self._task.args._add_sweeped_arg(self)
        if concat:
            self._ptrs.append(ptr)
        else:
...
class ArgumentParams(Parameters):
    ...
    def __repr__(self):
        ...
        for name, param in self:
            line = fmtstr.format(param._uname, param.value)
            ptr = param._ptr
...
class Task(BaseSweepIterator):
    ...
    def _call(self, loglevel=0):
        ...
        for name, arg in self.args:
            ...
            if arg in self.args._nested_args and loglevel is not None:
                ...
                if loglevel and loglevel <= levels.index(level):
                    continue
                sweep = ptr.sweep
                head = '    {}/{}: '.format(sweep.idx + 1, len(sweep))
                line = '{} = {!r}'.format(arg._uname, value)
                ...
                line = '{} = {!r}'
                line = line.format(arg._uname, value)
        ...
        for name, retval in self.returns:
            ...
            line = '        {} = {}'.format(retval._uname, value)
    ...
    def call(self, loglevel=0):
        ...
        line = '{} ({}/{})'.format(self.name, nested.idx + 1, len(nested))
        log.info(line)
